...
class ActionService:
    ...
    def get_frame_summary(self):
        ...
        This is the unified capture info + frame summary tool that includes:
        - API type, total actions, detailed action statistics (draw/dispatch/clear/copy/present/markers)
        - Top-level marker list, resource counts
        - Capture filename and driver info
        """
        if not self.ctx.IsCaptureLoaded():
            raise ValueError("No capture loaded")
        ...
        def callback(controller):
            ...
            # Capture filename (from context, no replay needed)
            filename = None
            try:
                import sys as _sys
                # Access ctx from outer scope via closure or pass in; try direct call
                filename = self.ctx.GetCaptureFilename()
            except Exception:
                pass

            # Driver info from structured file chunks
            driver = None
            try:
    ...
    def get_action_timings(
        ...
        Args:
            ...
            marker_filter: Only include actions under PushMarkers containing this string
                          (matches PushMarker name, not child action name).
